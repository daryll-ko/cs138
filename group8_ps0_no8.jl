...
# ╔═╡ 5a7c3795-70ba-4b69-b636-5c8e8cca46da
md"#1 and #3 are meta-concepts, #2 is an actual linear algebra concept."

# ╔═╡ 6cd4456c-5c1b-11ee-3a8b-eb49a1fb1869
md"## 1. Most of what we do in numerical computing is just taking advantage of what we already know."

# ╔═╡ cdc48aed-3b49-4b46-afc3-b9b50ce766ca
md"In the first few chapters of the series, Grant (3Blue1Brown) animates basic ideas in linear algebra as stepping stones for the next few chapters:"

# ╔═╡ 27632bd0-e185-4037-85e7-0ae2d3644780
html"""
...
<lite-youtube videoid=fNk_zzaMoSs params="modestbranding=1&rel=0"></lite-youtube>
"""

# ╔═╡ e8994a5e-c544-4246-895d-ba8da65d89dd
html"""
<div align="center"><i>Chapter 1.</i></div>
"""

# ╔═╡ cc751d2a-9d76-4f73-bb2f-d5612c5306c9
md"""
How does Grant manage to animate these ideas? If we think about it, simulations and approximations are inspired by **definitions**:

- We can create simulators for systems based on what we know about those systems.
- We can make numerical approximations for different operations based on what we know about those operations from previous Math subjects.
"""

# ╔═╡ b26084c7-5943-47b6-85fd-41fa3edfce81
md"Once we keep this in mind, we start to see subjects like CS 136/8 in a different light; *exploiting definitions* sounds more fun than just *drowning in math*."

# ╔═╡ 68f2e61c-d8ec-4f76-8f8f-6362ac160cf1
md"To illustrate, the next few sections will show how we can use basic definitions from calculus to approximate derivatives & integrals!"
...
md"""Now drag the slider below to change the value of ``\text{exp}`` and ``\Delta x = 10^{\text{exp}}`` (lower = more accurate):"""

# ╔═╡ f5d4935a-bf64-4938-809d-2b74fa2e1541
@bind exp Slider(-7:0, default = 0, show_value=true)
...
md"""Now drag the slider below to change the value of ``\text{exp}`` and ``n = 10^{\text{exp}}`` (higher = more accurate):"""

# ╔═╡ cb94ddfa-172e-4185-9d51-2d4823dec0f2
@bind exp₂ Slider(0:7, default = 0, show_value=true)
...
# ╔═╡ 5086f055-0546-44be-9a1e-b97ea838c77d
md"""
Chapter 3 covers one of the central ideas in linear algebra:

>> Matrices are linear transformations.

This idea is used in many fields adjacent to numerical computing, like computer graphics and machine learning. Understanding this role matrices play gives us another perspective on ideas like matrix multiplication and eigenvectors/eigenvalues.
"""

# ╔═╡ 18434c2f-75e7-4021-b93e-3a3610bb2779
md"Once we realize this, it's not so hard to see why the first third of CS 138 is all about matrices: they're *that* important!"
...
# ╔═╡ fb811106-4473-466a-997d-67392c4ab136
html"""
<div align="center"><i>Chapter 3.</i></div>
"""

# ╔═╡ b94e6244-c5da-4d5a-af4d-325b82b1c155
md"Are you ready for the fun part? Let's actually use some matrices!"
...
# ╔═╡ f4d9dabb-1e53-4b54-ba9e-969a4778c435
md"## 3. Animations make math feel much more intuitive."

# ╔═╡ c8d9eb01-893d-4931-880a-7786820e4c50
md"Grant's *Essence of Linear Algebra* series is amazing because not only does it explain linear algebra, but it also does so in a way that makes the viewer understand what's happening in real time through animations."

# ╔═╡ b2cf80ac-a90f-4098-b022-364b5b11f216
md"""As CS students, or as people exposed to programming, we also have the power to make use of the tools available to us to make things easier to understand. Things like a Python script or this Pluto notebook can help us in a variety of CS subjects (including 138!), and can make the ideas we come across feel more natural."""

# ╔═╡ 4fe157d3-1657-43ae-9ebc-2a95f15fed8d
md"To wrap up, here's a classic example that demonstrates both (frequentist) probability and approximating ``\pi``:"

# ╔═╡ 0137633c-fce9-4873-a1b9-6a920a281605
cm"""
...
``\text{area of a unit circle} = \pi``.

</div>

"""

# ╔═╡ bd6be185-78da-4213-8cdc-75f23db85176
md"Suppose we choose points at random inside the square ``[-1, 1] \times [-1, 1]``. We know that this square has area ``(1 - (-1))^{2} = 4``, so when we choose a point at random in this square, the probability that our point is also inside the unit circle is ``\dfrac{\pi}{4}``."

# ╔═╡ 7c34165a-c510-4f06-962d-a912c957a396
cm"""Using this idea, we can approximate ``\pi``! Suppose we choose a large number of points at random. Then by frequentist probability, we can say that

<div align="center">

``\dfrac{\text{\# of points inside unit circle}}{\text{\# of points we chose at random}} \approx \dfrac{\pi}{4}``.

</div>

"""

# ╔═╡ 1ded4413-8c83-411b-8052-6a91b83e01df
md"Let's see this in action! Drag the slider below to change the number of points we use:"

# ╔═╡ a53bc18d-3955-4f8f-aec1-ccc123d1efa6
@bind pts Slider(1:5*10^3, default = 100, show_value = true)

# ╔═╡ 101f2db7-4496-4df9-ae64-f8144da37068
L"""
\pi \approx %$(round(float(π), digits = 10))~\text{(actual)}
"""
...
# ╔═╡ fafb973b-c0e1-4398-ace5-b6a597eab2c4
det == 0 ? md"Oh, no! The determinant is 0... Try a different matrix?" : md""

# ╔═╡ b2f1da8b-cef5-4c46-ad8a-9bc8e341a083
L⁻¹ = inv(L)
...
T = transform(L⁻¹[1, 1], L⁻¹[1, 2], L⁻¹[2, 1], L⁻¹[2, 2])

# ╔═╡ d3ef7ff3-ae0b-4967-bf32-390b46af4f1b
img_sources = [
	"https://github.com/daryll-ko/cs138/blob/main/assets/black.png?raw=true" => "Blank Canvas",
	"https://github.com/daryll-ko/cs138/blob/main/assets/up_logo.png?raw=true" =>
	"UP Logo",
	"https://github.com/daryll-ko/cs138/blob/main/assets/scl_logo.png?raw=true" => "SCL Logo",
	"https://github.com/daryll-ko/cs138/blob/main/assets/pikachu.png?raw=true" => "Pikachu"
]

# ╔═╡ 0dcea395-8b61-4dfe-b7c5-a4c17380d766
md"""
Now choose an image to play with! (Continue changing the matrix around and see what happens.)
...
# ╔═╡ ac7f58b5-5426-48ca-b4e6-90e5fe28ac4d
TableOfContents()
...
# ╔═╡ 29f72140-b3f8-4919-8729-c56b1e892ae3
xs = [(rand() - 0.5) * 2 for _ in 1:pts]

# ╔═╡ 1da5ef21-b031-4879-bb3c-a94c549b2373
ys = [(rand() - 0.5) * 2 for _ in 1:pts]

# ╔═╡ 31861086-e68d-4df7-a050-8b3d8d3b75d9
unit_range = -1:0.01:1

# ╔═╡ 64664ae4-a385-47c9-9c89-d6080a483445
upper = map(x -> √(1-x^2), collect(unit_range))

# ╔═╡ 2e625ca4-955e-4d6b-a81e-675f8e02fe67
lower = upper .* -1

# ╔═╡ b1bf0cf9-8b60-4791-a610-5ea0b4c19800
begin
	plot(xs, ys, aspect_ratio = 1, seriestype=:scatter, legend = false, markersize = 2)
	plot!(unit_range, upper, color=:red, legend = false, linewidth = 4)
	plot!(unit_range, lower, color=:red, legend = false, linewidth = 4)
	xlims!(-1.5,1.5)
	ylims!(-1.5,1.5)
end

# ╔═╡ 3af9fd55-9456-467d-aa56-346dfc33c32a
begin
	inside = 0
	for i in 1:pts
		if hypot(xs[i], ys[i]) ≤ 1
			inside += 1
		end
	end
	inside
...
# ╔═╡ 97e01332-ce75-4194-9936-b7490319ceff
L"""
\pi \approx 4 \cdot \dfrac{\text{inside}}{\text{chosen}} = 4 \cdot \dfrac{%$(inside)}{%$(pts)} \approx %$(round(4 * inside/pts, digits = 10))~\text{(probabilistic)}
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "cbc77992ae8e1f6223a4b27a0c663504f9d7f868"
